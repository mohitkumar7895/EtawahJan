...
export async function getAnnouncements(): Promise<Announcement[]> {
  try {
    ...
    if (error.name === 'AbortError') {
      ...
      console.warn('Error fetching announcements (non-critical):', error.message || error);
    }
    return [];
  }
}

export async function createAnnouncement(announcement: Omit<Announcement, '_id' | 'id' | 'createdAt' | 'updatedAt'>): Promise<Announcement> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 8000);
  ...
  try {
    ...
    const data = await response.json();
    return { ...data.announcement, id: data.announcement._id || data.announcement.id };
  } catch (error: any) {
    clearTimeout(timeoutId);
...
export async function updateAnnouncement(id: string, announcement: Partial<Omit<Announcement, '_id' | 'id' | 'createdAt' | 'updatedAt'>>): Promise<Announcement> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    const errorMessage = errorData.message || errorData.error || 'Failed to update announcement';
    throw new Error(errorMessage);
  }
...
export async function deleteAnnouncement(id: string): Promise<void> {
  ...
  if (!response.ok) {
    const errorData = await response.json().catch(() => ({}));
    const errorMessage = errorData.message || errorData.error || 'Failed to delete announcement';
    throw new Error(errorMessage);
  }
}

// Public API: Get active announcements (for home page display)
export async function getActiveAnnouncements(): Promise<Announcement[]> {
  try {
    ...
    if (error.name === 'AbortError') {
      ...
      console.warn('Error fetching active announcements (non-critical):', error.message || error);
    }
    return [];
  }
}

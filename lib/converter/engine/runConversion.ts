...
import * as pdf from '../../../server/converter/processors/pdf';
...
const IMAGE_SHARP_TOOLS = new Set([
  'compress-image',
  'jpg-to-png',
  'png-to-jpg',
  'webp-to-jpg',
...
export async function runConversion(ctx: ProcessContext): Promise<ProcessResult> {
  ...
  if (IMAGE_SHARP_TOOLS.has(toolId)) {
    const image = await loadImageSharp();
    switch (toolId) {
      case 'compress-image':
        for (const inp of inputPaths) {
          const out = path.join(outputDir, outputName(inp, '-compressed' + path.extname(inp)));
      ...
      case 'jpg-to-png':
        for (const inp of inputPaths) {
          const out = path.join(outputDir, outputName(inp, '.png'));
      ...
      case 'png-to-jpg':
        for (const inp of inputPaths) {
          const out = path.join(outputDir, outputName(inp, '.jpg'));
      ...
      case 'jpg-to-webp':
        for (const inp of inputPaths) {
          const out = path.join(outputDir, outputName(inp, '.webp'));
  ...
  if (OFFICE_TOOLS.has(toolId)) {
    ...
    switch (toolId) {
      ...
      case 'ppt-to-pdf':
        for (const inp of inputPaths) {
          await addOutput(await office.officeToPdf(inp, outputDir));
        }
        break;
      case 'html-to-pdf':
        for (const inp of inputPaths) {
          await addOutput(await office.htmlToPdf(inp, outputDir));
        }
        break;
      case 'pdf-to-word':
        await addOutput(await office.pdfToOffice(inputPaths[0], outputDir, 'docx'));
        break;
      case 'pdf-to-excel':
        await addOutput(await office.pdfToOffice(inputPaths[0], outputDir, 'xlsx'));
        break;
      case 'pdf-to-ppt':
        await addOutput(await office.pdfToOffice(inputPaths[0], outputDir, 'pptx'));
        break;
    }
    onProgress(100, 'Done');
    return { outputs };
  ...
  switch (toolId) {
    ...
    case 'split-pdf': {
      ...
      for (const p of pages) await addOutput(p);
      try {
    ...
    case 'compress-pdf':
      for (const inp of inputPaths) {
        const out = path.join(outputDir, outputName(inp, '-compressed.pdf'));
    ...
    case 'pdf-to-jpg': {
      ...
      for (const p of imgs) await addOutput(p);
      try {
    ...
    case 'png-to-pdf': {
      ...
      await imagesToPdf(inputPaths, out);
      await addOutput(out);
      break;
    }
    case 'txt-to-pdf':
      for (const inp of inputPaths) {
        const out = path.join(outputDir, outputName(inp, '.pdf'));
    ...
    case 'batch-convert': {
      for (let i = 0; i < inputPaths.length; i++) {
        ...
        if (ext === '.pdf') {
          ...
          try {
            const image = await loadImageSharp();
            const out = path.join(outputDir, outputName(inp, '-compressed' + ext));
            await image.compressImage(inp, out, options.quality ?? 80);
            await addOutput(out);
          } catch {
            /* skip if sharp missing */
          }
        }
      }
      if (outputs.length > 1) {
